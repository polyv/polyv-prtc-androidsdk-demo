...
public class SelectAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private List<PRTCStreamInfo> mList;

    private SparseBooleanArray mSelectedPositions = new SparseBooleanArray();
    private boolean mIsSelectable = false;


    public SelectAdapter(List<PRTCStreamInfo> list) {
        if (list == null) {
    ...
    public void addStreamInfo(PRTCStreamInfo info, boolean needNotify){
        mList.add(info);
        if(needNotify)notifyDataSetChanged();
    }

    //更新adpter的数据和选择状态
    public void updateDataSet(ArrayList<PRTCStreamInfo> list) {
        this.mList = list;
...
//            ab.setTitle("已选择" + 0 + "项");
    ...
    //获得选中条目的结果
    public ArrayList<PRTCStreamInfo> getSelectedItem() {
        ArrayList<PRTCStreamInfo> selectList = new ArrayList<>();
    ...
    public void removeStreamInfoByUId(String uid){
        PRTCStreamInfo info = findStreamInfoByUId(uid);
        if(info != null){
            mList.remove(info);
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int i) {
        //设置条目状态
        PRTCStreamInfo streamInfo = mList.get(i);
